...
function buildPrompt() {
  const category = faker.helpers.arrayElement(['Code Review', 'Explain Concept', 'Write Email', 'Bug Report', 'Refactoring Plan', 'Feature Proposal']);

  const title = `${category}: ${faker.hacker.phrase()} ${faker.string.alphanumeric(4)}`;
  const content = [`Context: ${faker.lorem.sentences(2)}`, `Goal: ${faker.company.catchPhrase()}`, `Details: ${faker.lorem.paragraphs(2)}`].join('\n\n');
